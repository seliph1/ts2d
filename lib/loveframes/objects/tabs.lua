...
--]] ---------------------------------------------------------
	function newobject:draw()
		...
		if internals then
			for k, v in ipairs(internals) do
				local col = loveframes.BoundingBox(x + self.buttonareax, v.x, self.y, v.y, self.buttonareawidth, v.width,
					tabheight, v.height)
...
--]] ---------------------------------------------------------
	function newobject:wheelmoved(x, y)
		...
		if y < 0 then
			...
			local col = loveframes.BoundingBox(self.x, mx, self.y, my, self.width, 1, buttonheight, 1)
	...
	--[[---------------------------------------------------------
	- func: AddTab(name, object, image)
	- desc: adds a new tab to the tab panel
--]] ---------------------------------------------------------
	function newobject:AddTab(name, object, image, onopened, onclosed)
		...
		local tab = loveframes.objects["tabbutton"]:new(self, name, tabnumber, image, onopened, onclosed)
	...
	--[[---------------------------------------------------------
	- func: InsertTab(pos, name, object, image, onopened, onclosed)
...
--]] ---------------------------------------------------------
	function newobject:InsertTab(pos, name, object, image, onopened, onclosed)
		...
		local tab = self:AddTab(name, object, image, onopened, onclosed)
		local children = self.children
		local internals = self.internals
...
--]] ---------------------------------------------------------
	...
	- desc: sets the minimum width of the tab buttons
--]] ---------------------------------------------------------
	...
	--[[---------------------------------------------------------
	- func: GetMinWidth()
	- desc: gets the minimum width of the tab buttons
--]] ---------------------------------------------------------
