...
function newobject:RunKey(key, istext)
	...
	local onkeypressed = self.OnKeyPressed
	local oncontrolkeypressed = self.OnControlKeyPressed
	
	if not istext then
		if oncontrolkeypressed then
			oncontrolkeypressed(self, key)
		end
...
--[[---------------------------------------------------------
	...
	- desc: sets the object's text
--]]---------------------------------------------------------
function newobject:InsertText(text)
	...
	if multiline then
		...
		self.line = 1
		self.indicatornum = loveframes.utf8.len(self.lines)
	end
