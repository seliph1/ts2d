...
function lovernet.new(init)
  ...
  if self._type == lovernet.mode.client then
	...
	self.sendPacket = lovernet.sendPacket
  end
  ...
  self._serdes = init.serdes or require "lib/bitser"
  assert(self._serdes)
  ...
  self.isServer = lovernet.isServer
  
  self.waitForAnswer = lovernet.waitForAnswer
  self.answerHandler = lovernet.answerHandler
  self._waitingQueue = {}
  ...
  self.getPeers = lovernet.getPeers
...
--- Gets all peer objects
-- @return table of user objects
function lovernet:getPeers()
  return self._peers
end
...
function lovernet:getCache(name)
  assert(self._ops[name], string.format("Operation name %s does not exist",name))
...
--- Send a single packet to a peer index connected on the server.
-- @param name string
-- @param args data
-- @param peer_index number
-- @return boolean 
function lovernet:sendPacket(name, args, peer_index)
	assert(self._ops[name])
	local peers = self._peers
	if peers[peer_index] then
		local peer = peers[peer_index]
		local payload = { f = name, d = args}
		local packet = self:_encode { payload }
		
		peer:send( packet )
		
		self:log("event",string.format("Sent a message to peer ID:%s", peer_index))
		return true
	else
		self:log("error",string.format("Peer ID %s is unreachable", peer_index))
		return false
	end
end
...
--- Returns boolean
-- @return boolean 
function lovernet:isServer()
	return self._type == "Server"
end

--- Listens to any current operation that is on queue
-- @return boolean 
function lovernet:answerHandler()
	for index, v in pairs(self._waitingQueue) do
		local data_id = v.data_id
		local data = v.data
		local action = v.action
		
		self:pushData(data_id, data)
		local cache = self:getCache(data_id)
		if cache then
			local status, err = action(cache)
			
			self._waitingQueue[index] = nil
		end
	end
end

--- Add an operation to the queue to be listened
function lovernet:waitForAnswer(data_id, action, data)
	local payload = {
			data_id = data_id,
		data = data,
		action = action,
	...
	--table.insert(self.waitingQueue, payload)
	self._waitingQueue[data_id] = payload
end
