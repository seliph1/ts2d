---@diagnostic disable: undefined-field, param-type-mismatch, undefined-global, lowercase-global
--[[------------------------------------
LoveFS v1.1
...
function filesystem:loadImageData(source)
	source = source or self.selectedFile
	self.selectedFile = nil
	source = self:absPath(source)
	love.filesystem.createDirectory('lovefs_temp')
...
function filesystem:loadFile(source)
	source = source or self.selectedFile
	self.selectedFile = nil
	source = self:absPath(source)
	love.filesystem.createDirectory('lovefs_temp')
	self:copy(source, love.filesystem.getSaveDirectory() .. '/lovefs_temp/temp.file')
	return love.filesystem.newFileData('lovefs_temp/temp.file')
end
