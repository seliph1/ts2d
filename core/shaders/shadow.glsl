...
int tilemapWidth = int( iResolution.x / 32.0 );
int tilemapHeight = int( iResolution.y / 32.0 );
...
float sceneDist(vec2 p)
{
	...
	*/
	...
	float m = boxDist(translate(p, vec2(-32, -32)), vec2(16.0,16.0), 0.0);
	//float m = 0.0;
	int c = 0;
...
vec4 effect( vec4 fragColor, Image tex, vec2 fragCoord, vec2 screenCoord)
{
	...
	//vec2 p = fragCoord.xy;
	vec2 p = screenCoord.xy + vec2(0.5);
	...
	vec4 light1Col = vec4(1.0, 1.0, 1.0, 1.0);
	setLuminance(light1Col, 10.0);
	...
	// light
	col += drawLight(p, light1Pos, light1Col, dist, 300.0, 5.0);
	//col += drawLight(p, light2Pos, light2Col, dist, 200.0, 8.0);
	//col += drawLight(p, light3Pos, light3Col, dist, 300.0, 12.0);
	
	// shape fill
	//col = mix(col, vec4(0.0, 0.0, 0.0, 1.0), fillMask(dist));
	//col = mix(col, ambientColor, fillMask(dist));
